...
const REGION_INSIGHTS: Record<string, string> = {
  Alsace: 'Nestled between the Vosges mountains and the Rhine, Alsace is one of France\'s most distinctive wine regions. The mountains create a rain shadow effect, producing some of the driest conditions in France. The diversity of soils, including granite, limestone, sandstone, and clay, combined with Germanic varieties grown in French style produces wines of remarkable intensity and aromatic complexity.',
  ...
  Rioja: 'Spain\'s most celebrated wine region stretches along the Ebro River valley, sheltered by the Sierra de Cantabria mountains. Tempranillo, the dominant grape, blends with Garnacha, Mazuelo, and Graciano to produce wines that are classically aged in American and French oak. The Rioja classification system, comprising Crianza, Reserva, and Gran Reserva, reflects minimum aging requirements that contribute to the style\'s characteristic vanilla and leather complexity.',
...
function generateWineTastingNotes(color: string, fp: any, grapes: string[], region: any): string {
  if (color === 'red') {
    if (fp.tannins >= 7 && fp.body >= 7) {
      ...
      return `Expressive and approachable with generous ripe fruit character. The nose offers red and dark fruits, including cherry, blackberry, and plum, alongside subtle oak influence of vanilla and cedar. The palate is round and supple with velvety tannins that provide structure without austerity. Mid-palate flavors of mocha, dried herbs, and a touch of minerality add complexity. The finish is warm and persistent.`;
    } else {
...
function generateBeerTastingNotes(style: string, subStyle: string, description: string, abv: number, ibu: number): string {
  ...
  if (s.includes('wheat') || s.includes('weizen') || s.includes('hefeweizen')) {
    return `Hazy, pale yellow with a frothy white head that fills the glass with the characteristic aromas of this classic style, including banana, clove, and fresh-baked bread from the Bavarian yeast. On the palate, the beer is soft, pillowy, and refreshing with moderate carbonation. Flavors of wheat, citrus zest, and subtle yeast spice make this endlessly drinkable. The finish is clean, smooth, and gently warming.`;
  }
  ...
  if (s.includes('amber') || s.includes('red ale')) {
    return `Beautiful clear amber with a persistent off-white head. The nose balances caramel malt sweetness with toasted bread and light hop florals. On the palate, the medium-bodied malt profile of toffee, caramel, and biscuit is complemented by moderate hop bitterness and subtle citrus notes. The finish is balanced and moderately dry, making it an excellent everyday craft beer.`;
  }
  if (s.includes('cream ale') || s.includes('blonde') || s.includes('golden')) {
...
const STATIC_WINES: StaticWine[] = [
  ...
  { id: 2008, display_name: 'Barolo Castiglione', producer: 'Vietti', color: 'red', sub_type: 'DOCG', region: 'Piedmont', country_code: 'it', grapes: ['Nebbiolo'], description: 'The King of Italian wines in its most approachable form. Vietti\'s Castiglione blends fruit from three Barolo villages to create a wine of complexity and completeness, offering dried roses, tar, cherry, and a tannin grip built for the table.', pairing: 'Braised veal, beef cheek, black truffle, aged Castelmagno', fp: { sweetness: 1, acidity: 8, tannins: 9, alcohol: 8, body: 9, finish: 9 } },
  ...
  // ── France White ───────────────────────────────────────────
  { id: 2019, display_name: 'Puligny-Montrachet', producer: 'Domaine Leflaive', color: 'white', sub_type: 'Village', region: 'Burgundy', country_code: 'fr', grapes: ['Chardonnay'], description: 'From Burgundy\'s greatest white wine village, Domaine Leflaive is the reference producer. Biodynamic farming yields Chardonnay of peerless minerality and precision, with white flowers, hazelnut, lemon curd, and a chalky limestone backbone.', pairing: 'Lobster, scallops, sole meunière, poultry in cream sauce, brie', fp: { sweetness: 1, acidity: 8, tannins: 0, alcohol: 6, body: 7, finish: 9 } },
  { id: 2020, display_name: 'Riesling Clos Häuserer', producer: 'Zind-Humbrecht', color: 'white', sub_type: 'Vendanges Tardives', region: 'Alsace', country_code: 'fr', grapes: ['Riesling'], description: 'Alsace\'s most acclaimed estate coaxes Riesling to its greatest expression. The Clos Häuserer vineyard on limestone and marl soil produces a wine of electric intensity, with petrol, lime zest, ginger, and a seemingly endless mineral finish.', pairing: 'Foie gras, spiced dishes, Thai cuisine, soft ripe cheese', fp: { sweetness: 4, acidity: 9, tannins: 0, alcohol: 6, body: 6, finish: 9 } },
  { id: 2021, display_name: 'Sancerre Blanc', producer: 'Henri Bourgeois', color: 'white', sub_type: 'AOC', region: 'Loire Valley', country_code: 'fr', grapes: ['Sauvignon Blanc'], description: 'The benchmark Sancerre, from one of the Loire Valley\'s most respected estates. On the famous Kimmeridgian chalk soils, Henri Bourgeois produces Sauvignon Blanc of extraordinary purity, showing grapefruit, grass, white flowers, and flint.', pairing: 'Goat cheese, oysters, grilled sea bass, vegetable tart, asparagus', fp: { sweetness: 1, acidity: 9, tannins: 0, alcohol: 6, body: 5, finish: 8 } },
  { id: 2022, display_name: 'Gewurztraminer Hengst Grand Cru', producer: 'Josmeyer', color: 'white', sub_type: 'Grand Cru', region: 'Alsace', country_code: 'fr', grapes: ['Gewurztraminer'], description: 'The Hengst Grand Cru on limestone and clay produces Gewurztraminer of opulent aromatic richness, with lychee, rose petal, ginger, and Turkish delight rising from the glass in an almost exotic perfume. Full-bodied with a long, spiced finish.', pairing: 'Foie gras, Munster cheese, Thai curry, gingerbread, smoked salmon', fp: { sweetness: 5, acidity: 6, tannins: 0, alcohol: 7, body: 8, finish: 8 } },
